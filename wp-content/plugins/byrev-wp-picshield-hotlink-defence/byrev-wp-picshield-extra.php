<?php if(!defined('_BYREV_WP_PICSHIELD')) die('GTFO') ?>
	...
	<?php
		...
		$extra_code2 = htmlentities('<div style="text-align: center; line-height: 0px; font-size: 0px;" ><img src="<?=_BLANK_REFERRER_PNG_URL;?>" ></div>');
	?>
	...
	<div style="text-align: center; padding: 5px; margin 5px; border: 1px solid #aaa; background: #eee; ">
		...
		<img style="max-height:48px; max-width: 800px; width: 100%;" src="<?=_BLANK_REFERRER_DEMO_PNG_URL;?>" />
		<p>HTML + PHP code when plugin is active/enabled:</p>
		<textarea style="max-width: 1200px; width: 100%; height: 36px; padding: 5px;" ><?=$extra_code2;?></textarea>
		<p>HTML code when plugin is not active, but it is installed</p>
		<textarea style="max-width: 1200px; width: 100%; height: 42px; padding: 5px;" ><?=$extra_code1;?></textarea>
		<p>or use this simple link as image, anywhere in your template (when plugin active or not, but it is installed)</p>
		<textarea style="max-width: 1200px; width: 100%; height: 42px; padding: 5px;" ><?=_BLANK_REFERRER_PNG_URL;?></textarea>		
		<br /><br />
	</div>