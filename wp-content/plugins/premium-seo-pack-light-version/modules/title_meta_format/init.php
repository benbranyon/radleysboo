...
if (class_exists('pspTitleMetaFormat') != true) {
    class pspTitleMetaFormat
    {
        ...
        */
        public function __construct()
        {
			...
			foreach ( array('home', 'post', 'page', 'category', 'tag', 'taxonomy', 'archive', 'author', 'search', '404') as $k=>$v ) { //page types
	...
	return false;
		...
		public function the_url() {
			...
			} else if ( is_singular() ) { //post|page|post_type
		...
		public function the_canonical( $print=true ) {
			...
			$canonical = apply_filters( 'premiumseo_ft_canonical', $canonical );
		...
		protected function format_canonical() {
        	...
        	if ( is_singular() || $__postType == 'post' ) {
        	...
        	if ( is_singular() || is_category() || is_tag() || is_tax() || !empty($__postType) ) {
		...
		public function the_meta_robots() {
        	...
        	$m = $meta_robots;
        	$mi = isset($meta_robots['item']) ? $meta_robots['item'] : array();
        	$mt = isset($meta_robots['generaltag']) ? $meta_robots['generaltag'] : array();
			...
			$__meta_robots = apply_filters( 'premiumseo_ft_robots', $__meta_robots );
        ...
        public function the_title($title) {
        	...
        	$title = apply_filters( 'premiumseo_ft_title', $title );
        ...
        public function the_meta_description( $print=true ) {
        	...
        	$meta_desc = apply_filters( 'premiumseo_ft_meta_description', $meta_desc );
        ...
        public function the_meta_keywords() {
			...
			$meta_keywords = apply_filters( 'premiumseo_ft_meta_keywords', $meta_keywords );
        ...
        protected function the_format($field, $type) {
        	switch ($field) {
 			...
 			//format the page title!
        			$__return = $this->make_format($type, $__currentValue);
	...
	$value = '';
        	...
        	if ( is_singular() || is_category() || is_tag() || is_tax() || !empty($__postType) ) {
				...
				if ( $field=='all' )
					...
					if ( isset($__theMeta[ "$field" ]) ) {
						...
						if ( $field=='keywords' ) { //special case: keywords & focus keyword
        ...
        public function the_pagetype($field='title', $format_func='the_format') {
            ...
            if ( is_home() ) {
            	return call_user_func( array( $this, $format_func ), $field, 'home' );
            }
		...
		return call_user_func( array( $this, $format_func ), $field, 'category' );
            	...
            	return call_user_func( array( $this, $format_func ), $field, 'archive' );
            }
            else if ( is_author() ) {
            	return call_user_func( array( $this, $format_func ), $field, 'author' );
            }
            else if ( is_search() ) {
				return call_user_func( array( $this, $format_func ), $field, 'search' );
            }
            else if ( is_404() ) { 
				return call_user_func( array( $this, $format_func ), $field, '404' );
            }
        }
         ...
         *
         */
        protected function make_format($type='home', $theContent='') {
 			global $wp_query;
 			...
 			$__defaults = array( //default params!
				...
				'current_month' 		=> __( date( 'F' ), $this->the_plugin->localizationName ), //current month
				'current_week_day'		=> __( date( 'l' ), $this->the_plugin->localizationName ), //current week day
